...
###################################################################
####
#### after the final set of trait-associated haplotypes is defined,
#### now generate table of corresponding associated genomic regions
#### to be used for candidate gene search etc.
####
#### we go back to our already defined regions according to our GWAS results (based on LD; script 1.05)
#### and merge for each haplotype of the final list the corresponding regions of the environment-specific gwas
####
...
# input folder
infolder <- paste(TRAIT, "/QTLregs", sep = "")
infolder
...
for(qtl_i in finalQTLset){
	...
	if(is.null(Qeff_sd)){
		...
		for(env_i in unique(info_qtl$Environment)){
			Y_temp <- read.table(paste0(TRAIT, "/", list.files(path = TRAIT, ".pheno.txt")), stringsAsFactors = FALSE)
			colnames(Y_temp) <- Envs
			Y_temp <- Y_temp[ , env_i]			
			Qeff_sd <- c(Qeff_sd, info_qtl$Qeff[which(info_qtl$Environment == env_i)] / sd(Y_temp, na.rm = TRUE))
			names(Qeff_sd)[length(Qeff_sd)] <- env_i
